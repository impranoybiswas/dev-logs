...
const platformIcons: Record<string, React.ReactNode> = {
  ...
  github: <GithubOutlined className="text-foreground" />,
...
export default function SocialLinks({
  ...
  const mutation = useMutation({
    ...
    onSuccess: () => {
      message.success(
        `Social link ${editingLink ? "updated" : "added"} successfully`,
      );
    ...
    },
    {
      ...
      render: (url: string) => (
        <a
          ...
          className="text-primary truncate max-w-xs block"
        >
          {url}
        </a>
      ),
      ...
      <div className="flex flex-wrap gap-3 items-center">
        ...
        {editable && (
          <Button
            ...
            icon={<PlusOutlined />}
            onClick={handleAdd}
            className="flex items-center rounded-full px-5 py-2 h-auto text-sm font-medium text-primary border-primary/30 hover:border-primary/70"
          >
            Add Link
          </Button>
        )}
